<template>
  <div class="max-w-md mx-auto bg-white border-2 shadow-lg overflow-hidden md:max-w-2xl  p-5 m-5 ">
    <div class="flex justify-between">
      <img :src="image" alt="Car Image" class="w-50 h-48 object-cover">
      <div class="px-4 text-right">
        <v-badge
        v-if="booking.RentalStatus === 'Active'"
          color="success"
          content="Active"
          inline
        ></v-badge>
        <v-badge
        v-if="booking.RentalStatus === 'Closed'"
          color="error"
          content="Closed"
          inline
        ></v-badge>
        ...
        <p v-if="booking.RentalStatus === 'Closed'" class="text-sm text-gray-600">End Odometer: {{ booking.EndOdometer }}</p>
    ...
    <div v-if="isAdmin">
      <div class="flex">
        <button v-if="booking.RentalStatus === 'Active' && !showForm" @click="showForm=true" class="px-2 py-1 change-status text-white font-semibold border bg-gray-700 mt-5 rounded-lg">Close Booking</button>
      </div>
      <form v-if="showForm" @submit.prevent="closeBooking(booking)" class="text-left mt-10">
          <hr class="mb-5">
            <div class="mb-4 mr-5 w-1/2">
              ...
              <input type="number" class="w-full px-3 py-2 border rounded-md" v-model="endOdometerReading" id="endOdometerReading" name="endOdometerReading">
            </div>
